...
// pattern. Users is keyed by email (equals panel userId). Node is the aggregated
// node-level traffic derived from outbound stats.
type Snapshot struct {
	Users map[string]*UserTraffic
	Node  UserTraffic
}

// apiOutboundTag is the internal Xray API outbound tag; its counters are
// control-plane traffic and must not be attributed to node traffic usage.
