...
Last updated: `2026-04-20` (Phase 2A / Batch 1B-GRANT gate check)
...
| Current automation control surface cannot execute Coolify GitHub provider re-authorize/rebind directly | Batch 1B-GRANT can stall despite known blocker and known target repo | Medium | Perform grant repair in Coolify UI (provider auth + repo include), then re-check gate from API evidence | Keep source on current healthy repo and do not retry Batch 1B until selector proof is captured | Open (Execution-channel limitation) |
